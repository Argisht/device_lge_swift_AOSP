...
status_t AudioHardware::setAmpGain(int value)
{
     struct msm_snd_set_amp_gain_param args;
     args.value = value;

     if (ioctl(m7xsnddriverfd, SND_SET_AMP_GAIN, &args) < 0) {
         LOGE("snd_set_amp_gain error.");
         return -EIO;
     }
    return NO_ERROR;
}

status_t AudioHardware::setMicAmpGain(int value)
{
     struct msm_snd_set_micamp_gain_param args;
     args.value = value;

     if (ioctl(m7xsnddriverfd, SND_SET_MICAMP_GAIN, &args) < 0) {
         LOGE("snd_set_micamp_gain error.");
         return -EIO;
     }
    return NO_ERROR;
}
